...
public class Main extends JFrame implements ActionListener {
    ...
    public static void main(String[] args) throws IOException {
    	...
    	//GUI implementation for intput----------------------------------------------------------
        container = new JPanel();//-----
        container.setLayout(new BoxLayout(container,BoxLayout.X_AXIS));//-----
        inputPanel = new JPanel();
        container.add(inputPanel);
        
        inputFrame = new JFrame(); //Initializes the input display Frame
        inputFrame.setSize(1920, 1080); //Configures the input Frame size
        inputFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//Configures the input Frame closing protocal 
        inputFrame.add(container); 
        inputPanel.setLayout(null); //Configures the Layout of the First input Panel


        titleLabel = new JLabel("Java-to->Python"); //Initializes a Header label
        titleLabel.setBounds(100, 20, 350, 20); //Configures the Header ladel bounds 
        inputPanel.add(titleLabel); //Adds the title label to the input Panel

        displayButton = new JButton("GO"); //Initializes a Button to display results upon being clicked
        displayButton.setBounds(50, 90, 100, 25); //Configures the bounds for the display button
        displayButton.addActionListener(new Main()); //Adds an Action Listener to the display button 
        inputPanel.add(displayButton); //Adds the display button to the input Panel
        ...
        fileLabel = new JLabel("Which file would you like to translate?"); //Initializes a prompting label. Prompts users for file to translate.
        fileLabel.setBounds(20, 60, 300, 25); //Configures the file prompt label bounds 
        inputPanel.add(fileLabel); //Adds the file prompt label to the input Panel
        
        fileName = new JTextField(20); //Initializes the text field for the user to select a file name
        fileName.setBounds(275, 60, 120, 25); //Configures the input text field bounds
        fileName.addActionListener(new Main()); //Adds on Action Listener to the the text field
        inputPanel.add(fileName); //Adds the file text field to the input Panel

        inputFrame.setVisible(true); //Configures the input Frame to be visible
